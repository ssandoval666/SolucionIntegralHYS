...
namespace WebAppSmartHYS.Controllers
{
	...
	[AllowAnonymous]
	public class ConstanciasCapacitacionesController : Controller
	{
		...
		public ConstanciasCapacitacionesController(IWebHostEnvironment webHostEnvironment, IConfiguration configuration)
		{
		...
		[HttpGet]
		public JsonResult GetConstancias()
		{
			...
			try
			{
				...
				var vRta = new { };
				List<Constancias> oRta = new List<Constancias>();

				using (var ds = new DataSet())
				{
					using (var da = new SQLiteDataAdapter("SELECT * FROM Capacitaciones", "Data Source=app.db; Version = 3; New = True; Compress = True; "))
					{
						using (var tran = new TransactionScope())
						{
							...
							if (dt.Rows.Count > 0)
							{
								...
								{
									var oCons = new Constancias();
									oCons.documento = ((System.Data.DataRow)row).ItemArray[0].ToString();
									oRta.Add(oCons);
		...
		private byte[] GenerarPDF(string Documento)
		{
			...
			byte[] oRta = null;
			string Empleado = "", Capacitacion, TipoCapacitacion, PorcentajeCalificacion;
					...
					using (var tran = new TransactionScope())
					{
						...
						if (dt.Rows.Count > 0)
						{
							...
							foreach (var row in dt.Rows)
							{
								...
								Empleado = ((System.Data.DataRow)row).ItemArray[1].ToString();								
								binarySignature = Convert.FromBase64String(((System.Data.DataRow)row).ItemArray[4].ToString());
		...
		public JsonResult GrabarPDF(string Empleado, string Documento, string Capacitacion, string TipoCapacitacion, string PorcentajeCalificacion, string FirmaEmpleado)
		{
			...
			try
			{
				...
				byte[] pdfBytes = GenerarPDF(Documento);
